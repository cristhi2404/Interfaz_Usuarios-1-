    ...
    if (btnCambiarFotoAgg && inputFotoAgg && divFotoPreviewAgg) {
        ...
        inputFotoAgg.addEventListener("change", () => {
            ...
            reader.onload = (e) => {
                ...
                divFotoPreviewAgg.style.backgroundSize = "countain";
                divFotoPreviewAgg.style.backgroundRepeat = "no-repeat";
        ...
        if (btnEliminarFotoAgg) {
            const eliminarFotoHandler = (e) => {
                ...
                inputFotoAgg.value = "";
                // Imagen por defecto
                divFotoPreviewAgg.style.backgroundImage = "url('img/logo_defecto.png')";
...
async function agregarUsuario() {
    ...
    const forma_acceso = document.getElementById("forma_accesoagguser").value;    
    const url = document.getElementById("urlagguser").value;
    if (!nombre || !tipoaplicacion || tipoaplicacion == "0" || !descripcion || !forma_acceso) {
    ...
    datos.append("id_tipo_aplicacion", tipoaplicacion);
    datos.append("descripcion", descripcion);
    datos.append("forma_acceso", forma_acceso);
    ...
    try {
        const response = await fetch('Crearcrudapis.php', {
            method: 'POST',
            body: datos
        });
